...
controlador:
	in $0x60, %al         # lee código de búsqueda
	xor %ch, %ch
	mov %al, %cl          # copia código

	mov $0x0f, %ah        # color: blanco sobre negro

	mov %cx, %si          # dígito hexadecimal
	shr $4, %si           # primer carácter
	movb hex(%si), %al    # al = hex[al]
	stosw                 # imprimir carácter: ax --> es:di++

	mov %cx, %si          # dígito hexadecimal
	and $0xf, %si         # segundo carácter
	movb hex(%si), %al    # al = hex[al]
	stosw                 # imprimir carácter: ax --> es:di++
